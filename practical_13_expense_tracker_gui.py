...
class ExpenseTracker:
    ...
    def add_expense(self):
        ...
        if category and amount.replace('.', '', 1).isdigit():
            new_row = pd.DataFrame([[category, float(amount)]], columns=["Category", "Amount"])
    ...
    def show_report(self):
        ...
        messagebox.showinfo("Report", f"{report.to_string()}")
        report.plot(kind='bar', color='orange')
